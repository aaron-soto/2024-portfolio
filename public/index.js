...
class Particle {
  ...
  update() {
    if (
      this.x + this.size > canvas.width / (window.devicePixelRatio || 1) ||
      this.x - this.size < 0
    ) {
      this.dx = -this.dx;
    }
    ...
    if (
      this.y + this.size > canvas.height / (window.devicePixelRatio || 1) ||
      this.y - this.size < 0
    ) {
      this.dy = -this.dy;
    }
...
for (let i = 0; i < particleCount; i++) {
  const size = 3;
  const x =
    Math.random() * (canvas.width / (window.devicePixelRatio || 1) - size * 2) +
    size;
  const y =
    Math.random() *
      (canvas.height / (window.devicePixelRatio || 1) - size * 2) +
    size;
